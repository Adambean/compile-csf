...
foreach ($servers as $s => $server) {
    ...
    try {
        ...
        if (is_iterable($directoryCsfBaseContents = scandir($directoryCsfBase))) {
                            ...
                            if (is_iterable($sContents)) {
                                foreach ($sContents as $l => $line) {
                                    ...
                                    foreach ($server["csfConf"] as $confKey => $confValue) {
                                        if (null === $confValue || !is_scalar($confValue)) {
                                            continue;
                                        }

                                        $confValue      = trim(strval($confValue));
            ...
            printf("- - - Build skipped: Directory could not be scanned.\n");
        }
        // >> Build: CSF/LFD
        ...
        if ($enableUpload) {
            ...
            if (is_iterable($directoryCsfContents = scandir($directoryCsfL))) {
                ...
                printf("- - - Uploading skipped: Directory could not be scanned.\n");
            }
        }
        // >> Upload: CSF/LFD

        // << Restart: CSF/LFD
        if ($enableRestart && isset($server["csfConf"]) && is_array($server["csfConf"])) {
            printf("- Restarting CSF & LFD...\n");
            $binCsf = "/usr/sbin/csf";
            if (!isset($server["csfConf"]["CSF"]) || !is_string($server["csfConf"]["CSF"]) || "" === ($binCsf = trim($server["csfConf"]["CSF"]))) {
            ...
            if (!is_resource($csfRestartOut) || false === stream_get_contents($csfRestartOut)) {
